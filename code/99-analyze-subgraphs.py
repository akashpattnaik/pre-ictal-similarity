...
for index, row in patient_cohort.iterrows():
    ...
    for i_comp in range(n_components):
        fig, ax = plt.subplots()
        im = ax.imshow(component_z[i_comp, :, sort_soz_inds].T)
        ...
        if SAVE_PLOT:
            plt.savefig(ospj(pt_figure_path, "soz_subgraph_{}_heatmap.svg".format(i_comp)))
            plt.savefig(ospj(pt_figure_path, "soz_subgraph_{}_heatmap.png".format(i_comp)))
            plt.close()
